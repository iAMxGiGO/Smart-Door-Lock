...
def guiEventHandler(num):
    ...
    if (num == -1):
        ...
        print(passcode)
        rows = -1
        searchForPasscodeQuery = "select username , name , Mobile from users WHERE Passcode = "+passcode+";"
        mycursor.execute(searchForPasscodeQuery)
        for username, name, mobile in mycursor:
            print("User found in the database: Username:{}  Name:{} Mobile: {}".format(username, name, mobile))
        ...
        for addr, name in nearby_devices:
            try:
                ...
                searchForBluetoothQuery = "select username , name from users WHERE Bluetooth = '"+str(addr)+"';"
                print(searchForBluetoothQuery)
                mycursor.execute(searchForBluetoothQuery)
                for i in mycursor:
                    print(i)
                rows = mycursor.rowcount
                if rows == -1:
...
btnClear = Button(gui, command=lambda:guiEventHandler(-2), padx=7, pady=28, fg='black', font=('times new roman', 12, 'bold'), text='Clear').grid(row=5, column=0)
btnOTP = Button(gui, command=lambda:guiEventHandler(-3), padx=9, pady=28, fg='black', font=('times new roman', 12, 'bold'), text='OTP').grid(row=5, column=2)
btnEnter = Button(gui, command=lambda:guiEventHandler(-1), padx=95, pady=10, fg='black', font=('times new roman', 12, 'bold'), text='Enter', justify='center').grid(row=6, column=0,columnspan=3)
btnRFID = Button(gui, command=lambda:guiEventHandler(-4), padx=95, pady=10, fg='black', font=('times new roman', 12, 'bold'), text='RFID', justify='center').grid(row=7, column=0,columnspan=3)
btnBLE = Button(gui, command=lambda:guiEventHandler(-5), padx=82, pady=10, fg='black', font=('times new roman', 12, 'bold'), text='Bluetooth', justify='center').grid(row=8, column=0,columnspan=3)
